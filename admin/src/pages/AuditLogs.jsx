        ...
        <div className="w-full px-4 py-8 md:px-8">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Track administrative actions and system modifications.</p>
                </div>
                <button
                    onClick={clearLogs}
            ...
            <div className="w-full overflow-hidden rounded-[28px] border border-slate-100 bg-white shadow-sm">
                ...
                {loading ? (
                    ...
                    <div className="px-8 py-16 text-center text-sm text-slate-400 font-medium italic">All quiet. No activity recorded yet.</div>
                ) : (
                    <div className="divide-y divide-slate-50">
                        {list.map((log) => (
                            <div key={log._id} className="grid md:grid-cols-[180px_140px_1fr_180px] items-center px-8 py-5 transition-colors hover:bg-slate-50/30">
                                <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
                                    <Calendar size={14} className="text-slate-300" />
                                    {new Date(log.timestamp).toLocaleString('vi-VN', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
                                </div>
                                <div>
                                    <span className={`rounded-xl border px-3 py-1 text-[10px] font-bold uppercase tracking-widest ${getActionColor(log.action)}`}>
                                        {log.action}
                                    </span>
                                </div>
                                <div className="text-sm font-medium text-slate-600 line-clamp-1">{log.description}</div>
